...
Generated by `run/make_report.py` from `results/metrics_aggregated.csv` + `results/stability.csv`. This page shows **SNR=1** (realistic additive noise). Reproduce the full SNR sweep with `python run/benchmark.py --dataset all --methods all --seeds 3 --dim 768 --n 1000 --snr inf 4 1` (or just the reported level with `--snr 1`), then rebuild this page with `python run/make_report.py`. Code, data, and the full methodology (`README.md`) live in the [GitHub repository](https://github.com/toorpia/dr-fidelity-benchmark).
