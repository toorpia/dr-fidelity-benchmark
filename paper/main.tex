...
% ============================================================
% DR Fidelity Benchmark --- arXiv preprint
% Author: Yoshio Takaeda (toor Inc.)
% Source of record: https://github.com/toorpia/dr-fidelity-benchmark
% Frozen snapshot: release tag v1.2.0  +  Zenodo DOI 10.5281/zenodo.21189374
% ------------------------------------------------------------
...
% \usepackage{lmodern}  % re-enable for arXiv (present there); omitted for local build
...
\usepackage{orcidlink}   % if unavailable at build time, comment out and remove \orcidlink uses

\graphicspath{{figures/}}

% --- editorial marker for in-progress spots (remove before submission) ---
\newcommand{\TODO}[1]{\textcolor{red}{[\textbf{TODO:} #1]}}
\newcommand{\rhoS}{\ensuremath{\rho}}

\title{\textbf{A Fixed-Radius Distance-Band Benchmark for\\
Dimensionality-Reduction Fidelity, and a\\
Citable Characterization of a Closed-Source Method}}
% NOTE: title is a working title --- shorten/adjust to taste.

\author{
  Yoshio Takaeda\,\orcidlink{0009-0005-4968-9456}\\
  \textit{toor Inc.}\\
  \texttt{takaeda@sophia.ac.jp}
  % NOTE: decide sophia.ac.jp vs. a toor Inc. address for the paper contact
}
...
seven methods --- PCA, Isomap, t-SNE, UMAP, PyMDE, PCC, and the closed-source
\textbf{toorPIA} --- and show that (i) high global Shepard $\rho$ can coexist
...
direction the bias predicts; and (iii) a membership-restricted Shepard $\rho$
resolves single-point and minority-population questions that many-pair
statistics cannot. A supplementary out-of-sample (\emph{addplot}) test poses
...
benchmark seven methods, including the closed-source toorPIA, and release
...
\begin{enumerate}
  ...
  \item \textbf{A seven-method benchmark on known-geometry synthetic data} at a
...
stress-based MDS). PCC \cite{pcc2025} maximizes the correlation between
high-D and 2-D distances to a sampled reference set --- a distance-fidelity
objective with sparse constraints. toorPIA is a closed-source commercial
...
\cite{espadoto2019survey}. Our band-restricted Shepard $\rho$ stays in the
...
% --- SOURCE: REPORT.md "Reading guide" + "Methodology notes" + README "Metrics"
% This is the methodological core --- write it carefully and NON-defensively.
...
$\alpha$ is the optimal global scale. Stress catches methods that keep the
...
to 1 point to the 5th (6th and 7th receive 0). The composite $\Sigma$ is the
...
and toorPIA are deterministic and show point values. When two methods' CIs
overlap on a metric, no strict winner is asserted. Outright failures are
...
The seven methods are PCA \cite{hotelling1933pca}, Isomap
...
perplexity 30, UMAP \texttt{n\_neighbors} 15; PCC with its published Pearson
loss and reference-set size equal to $N$; PyMDE with an absolute-distance
loss). toorPIA is called through its embedding endpoint
...
committed hyperparameter-sensitivity sweep (each neighbor method's
placement-critical knob over 5--100) shows that no setting changes the
composite leaders, although tuned t-SNE's near-band standing on the density
dataset improves --- a genuine sensitivity, disclosed in the artifact.
...
All ranking tables report the committed v1.2.0 results at SNR${=}1$
...
($\Sigma{=}9$: best near-band $\rho$ 0.312, second-best global $\rho$ 0.815)
...
construction). toorPIA tops the composite ($\Sigma{=}9$: best global $\rho$
0.586, second-best near band); t-SNE leads the near band and places second
($\Sigma{=}6$); PCC again pairs a competitive global $\rho$ with an
...
extent, t-SNE and UMAP draw them as compact dots, PCC as needle-thin
...
\emph{not} win the composite: PCC is first ($\Sigma{=}8$) and t-SNE second
($\Sigma{=}7$), while toorPIA holds the best global $\rho$ (0.729) but its
first-mode near $\rho$ ranks sixth, dropping it to third
(Table~\ref{tab:transition}). The embedding gallery
...
connecting bridges, fragmenting the loop; PCC draws the seven states as radial
spokes fused at a central hub, so every state becomes adjacent to every other
and the cyclic adjacency is lost. We report both readings --- the quantitative
composite in which toorPIA is third, and the qualitative gallery in which it is
...
\begin{figure}[t]
  ...
  both; the other methods each recover at most one (see
...
scores as success. PCC keeps the bulk clean but tears same-kind pairs apart
...
UMAP are the mirror image --- the best minority internals (0.658 / 0.451) with
cross-population $\rho \approx 0$: the minority's placement carries no
distance information. PCC fails both (0.023 / 0.198) --- and the failure is
...
% NOTE: membership-restricted rho values above are from
% results/metrics_aggregated.csv (within_minority_shepard__vs_ambient,
% cross_population_shepard__vs_ambient); the ranking table (tables/populations)
% carries the standard two-sided columns.
...
the headline metrics, and is committed per dataset in the artifact
(\texttt{results/stability.csv}). PCA, Isomap, and toorPIA are deterministic
(same input $\to$ identical map; toorPIA's embedding endpoint exposes no
seed). The stochastic methods' wobble is small relative to the between-method
...
Three of the seven methods --- t-SNE (as implemented in scikit-learn), PyMDE,
...
and UMAP the anomalies land \emph{inside or at} the normal clusters (median
radius ratios 0.97--1.34, minima down to 0.28): the anomaly is drawn as just
another normal point of its source cluster, so the nominally high attribution
accuracy (0.8--1.0) carries no alarm --- detection silently fails. Added
normal controls land correctly for every operable method (ratio 0.87--1.01).

Three honest notes on the protocol. (1) toorPIA's \texttt{addplot\_embedding}
targets the fitted basemap's server-side state, so this test performs a live
\texttt{basemap\_embedding} plus one \texttt{addplot\_embedding} call per
added point (the monitoring semantics: points arrive one at a time) and
...
receive identical treatment. (2) The PCA/Isomap transforms and toorPIA are
deterministic; UMAP's transform is seeded. (3) A re-fit-based alternative for
...
% --- SOURCE: REPORT.md "Scope" + README "Conflict of interest & trust measures"
% These pre-written passages are strong; adapt them faithfully.
...
The transparency this buys is visible in the results themselves: t-SNE leads
the first-mode near band on four of the five datasets (toorPIA takes it only
on density); on the transition dataset the composite goes to PCC with
toorPIA third (\S\ref{sec:transition}); and the hyperparameter-sensitivity
sweep discloses that tuned t-SNE overtakes toorPIA's near-band $\rho$ on
density (\S\ref{sec:methodsconfig}). These unfavorable readings are printed
...
On five known-geometry datasets at SNR${=}1$, the seven-method benchmark
...
the methods the $k$-NN reference favors; and, out of sample, one method
...
leads the composite on four of the five datasets but drops to third on
transition, and t-SNE owns the near band nearly everywhere --- and the
...
coordinates, the release (v1.2.0, archived on Zenodo) is intended as a
...
(release tag v1.2.0), archived at
Zenodo (\href{https://doi.org/10.5281/zenodo.21189374}{10.5281/zenodo.21189374}).
This paper reports the results committed at that tag.

\section*{Acknowledgments}
\TODO{optional --- NAIST (Prof.\ Taro Watanabe's NLP lab), Sophia University,
any colleagues; omit if not desired.}

% ============================================================
\bibliographystyle{unsrt}   % citation-order numbered; swap to plainnat/abbrvnat if desired
